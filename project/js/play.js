...
function setup() {
  ...
  //alert(loggedinUser);
   ...
   selectedStory=getUrlVars()["s"];
    if(selectedStory === undefined){
     //selectedStory = localStorage.getItem("selectedStory");
	  ...
	  localStorage.setItem("selectedStory",selectedStory); //storyIdArray
      selectedStory = localStorage.getItem("selectedStory");
   ...
   game.on('value', function(snapshot){
	  ...
	  // update to DB only when
       if(loggedinUser === 'scrummaster' ){	  
          database.ref('games/'+ gameId).update({
		   ...
		   // selectedStory = snapshot.val().currentStory;
   ...
   var closeModalButton = select('#closeModalButton');
	...
	var inviteTeamButton =  select('#inviteTeam');
	inviteTeamButton.mousePressed(inviteTeamFunc);
...
function inviteTeamFunc(){
	
	prompt('please share this link :\n', window.location.href);
...
function flipcardsEvent(){
	...
	//var query = database.ref('games/'+ gameId+'/story_users/'+selectedStory).orderByChild('point');
	// query.on('child_added', function(snapshot){
	//	   var storyUser = snapshot.val();
	//	   console.log(storyUser.point,storyUser.name);
	//   });  
...
function sumPoints(){
	...
	var pointArr = [];
	var $avg,
  getAvg = function(selector, attrName) {
      return $(
        $(selector).toArray().forEach(function(a){
            pointArr.push(parseInt(a.getAttribute(attrName)));
        })
      );
  };

 getAvg('#playerContainer .card', 'data-order');
 ...
 if(pointArr.length>0){
	...
	console.log(sum);console.log(pointArr.length);
	console.log(sum/pointArr.length);
	console.log(Math.ceil(sum/pointArr.length));
	var score = Math.ceil(sum/pointArr.length);
	document.getElementById('storyScore').value = score;
	
	// change background and 
	$("#storyScore").addClass("bg-warning").fadeIn(5000);
	$("#storyScoreButton").addClass("btn-success");
	
	setTimeout(function() {
      $("#storyScore").removeClass("bg-warning");
	  $("#storyScoreButton").removeClass("btn-success");
     }, 5000);
     }
...
$sorted_items = getSorted('#playerContainer .card', 'data-order').clone();

$('#playerContainer').html( $sorted_items );
...
function populateStoryList2(){
   ...
   for (var i = 0; i < storyIdArray.length; i++) {
	   if(storyIdArray[i] == selectedStory){
		 ...
		 html.push('<li class="list-group-item d-flex justify-content-between align-items-center list-group-hover">'); 
	   }
...
function populatePlayContainer(storyUserArray){
  ...
  var html =[];	
  var htmlTemp = '<div data-order="[i]" class="card text-black border-primary mb-3" style="max-width: 10rem;margin-left:2%;">'+
	 ...
	 if(storyUserArray!=null && storyUserArray.length>0){		 
		for(var i=0; i <storyUserArray.length; i++){
	       ...
	       html.push(htmlTemp.replace("[i]",storyUser.point).replace("[i]",storyUser.point).replace("[name]",storyUser.name).replace("[name]",storyUser.name));
		}
	}
...
function closeModal(){
	...
	//set default story as first story
	localStorage.setItem("selectedStory",storyIdArray[0]); //storyIdArray
